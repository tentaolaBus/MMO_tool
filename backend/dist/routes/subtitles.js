...
const subtitleGenerator_1 = require("../services/subtitleGenerator");
const translator_1 = require("../services/translator");
const database_1 = require("../services/database");
const subtitleRenderer_1 = require("../services/subtitleRenderer");
const router = (0, express_1.Router)();
/**
 * GET /api/clips/:clipId/subtitles?lang=en
 * Get subtitles for a clip in specified language
 */
router.get('/:clipId/subtitles', async (req, res) => {
    ...
    try {
        ...
        if (clip) {
            ...
            const clipsMetadataPath = path_1.default.resolve('./storage/clips', `${jobId}_metadata.json`);
            if (fs_1.default.existsSync(clipsMetadataPath)) {
                const metadata = JSON.parse(fs_1.default.readFileSync(clipsMetadataPath, 'utf-8'));
                const clipMeta = metadata.clips?.find((c) => c.clipIndex === clipIndex);
                if (clipMeta) {
        ...
        const cacheDir = path_1.default.resolve('./storage/subtitles');
        if (!fs_1.default.existsSync(cacheDir)) {
            fs_1.default.mkdirSync(cacheDir, { recursive: true });
        }
        // Try edited version first, then original
        const cacheFileBase = `${jobId}_${clipIndex}_${language}`;
        const editedCacheFile = path_1.default.join(cacheDir, `${cacheFileBase}_edited.json`);
        const cacheFile = path_1.default.join(cacheDir, `${cacheFileBase}.json`);
        // Return edited cached subtitles if available
        if (fs_1.default.existsSync(editedCacheFile)) {
            const cached = JSON.parse(fs_1.default.readFileSync(editedCacheFile, 'utf-8'));
            return res.json({
                success: true,
                ...cached,
                cached: true,
        ...
        const subtitles = await (0, subtitleGenerator_1.generateSubtitlesForClip)(jobId, `${jobId}_${clipIndex}`, // Use consistent clipId format for generator
        clipStartTime, clipEndTime, 'en' // Always generate in English first
        );
        // Translate if needed
        if (language !== 'en' && subtitles.segments.length > 0) {
            const translatedTexts = await (0, translator_1.translateSegments)(subtitles.segments, language);
        ...
        // Cache the result
        fs_1.default.writeFileSync(cacheFile, JSON.stringify(subtitles, null, 2));
        res.json({
            success: true,
            ...subtitles,
            cached: false,
 ...
 */
router.put('/:clipId/subtitles', async (req, res) => {
    const { clipId } = req.params;
    ...
    try {
        ...
        const editedFile = path_1.default.join(cacheDir, `${clipId}_${language}_edited.json`);
        const editedSubtitles = {
            clipId,
        ...
        fs_1.default.writeFileSync(editedFile, JSON.stringify(editedSubtitles, null, 2));
...
// ────────────────────────────────────────────────────────────────────────
const STYLES_DIR = path_1.default.resolve('./storage/subtitle-styles');
/**
 * GET /api/clips/:clipId/subtitle-style
 * Load saved subtitle style for a clip
 */
router.get('/:clipId/subtitle-style', (req, res) => {
    const { clipId } = req.params;
    try {
        if (!fs_1.default.existsSync(STYLES_DIR)) {
            return res.json({ success: true, style: null });
        }
        const filePath = path_1.default.join(STYLES_DIR, `${clipId}.json`);
 ...
 */
router.post('/:clipId/subtitle-style', async (req, res) => {
    const { clipId } = req.params;
    ...
    try {
        ...
        const originalClipDir = path_1.default.resolve('./storage/clips');
        const originalFilename = `clip_${clip.job_id}_${clip.clip_index}.mp4`;
        let clipPath = path_1.default.join(originalClipDir, originalFilename);
        console.log(`🔍 Save Style — resolving clip path:`);
        console.log(`   DB video_path:     ${clip.video_path}`);
        console.log(`   Convention path:   ${clipPath}`);
        console.log(`   Convention exists: ${fs_1.default.existsSync(clipPath)}`);
        ...
        if (!fs_1.default.existsSync(clipPath)) {
            ...
            console.log(`   Fallback exists:   ${fs_1.default.existsSync(clipPath)}`);
        }
        if (!fs_1.default.existsSync(clipPath)) {
            ...
            return res.status(404).json({
                ...
                hint: `Tried: ${path_1.default.join(originalClipDir, originalFilename)} and ${clip.video_path}. Re-render clips to regenerate files.`,
            });
        }
        const subtitleEnabled = enabled !== undefined ? enabled : true;
        // ── STEP 2: Save style config ───────────────────────────────────
        if (!fs_1.default.existsSync(STYLES_DIR)) {
            fs_1.default.mkdirSync(STYLES_DIR, { recursive: true });
        }
        const styleFilePath = path_1.default.join(STYLES_DIR, `${clipId}.json`);
        fs_1.default.writeFileSync(styleFilePath, JSON.stringify({
            clipId,
            style,
        ...
        if (subtitleEnabled) {
            ...
            let subtitlesFile = path_1.default.join(subtitlesDir, `${cacheKey}_edited.json`);
            if (!fs_1.default.existsSync(subtitlesFile)) {
                subtitlesFile = path_1.default.join(subtitlesDir, `${cacheKey}.json`);
            }
            if (fs_1.default.existsSync(subtitlesFile)) {
                const data = JSON.parse(fs_1.default.readFileSync(subtitlesFile, 'utf-8'));
        ...
        const finalVideoPath = await subtitleRenderer_1.subtitleRenderer.renderWithSubtitles(clipPath, segments, clipId, style, subtitleEnabled && segments.length > 0);
        const renderTime = ((Date.now() - startTime) / 1000).toFixed(1);
        const finalFilename = path_1.default.basename(finalVideoPath);
        ...
        await database_1.queries.updateClipVideoPath(finalVideoPath, clipId);
        // ── DIAGNOSTIC: Verify pipeline integrity ───────────────────────
        const newFileSize = fs_1.default.existsSync(finalVideoPath) ? fs_1.default.statSync(finalVideoPath).size : 0;
